...
class PSN:
    ...
    def requester(self, method, api, **kwargs):
        ...
        if response.status_code >= 400:
            ...
            try:
                ...
                pass
            raise Exception(
                f"Error {method} {api} {response.status_code}: {error_message}"
            )
        return response
    ...
    def check_access_token(self):
        if not self.access_token:
            return False
        params = {"fields": "npId,onlineId,accountId,avatarUrls,plus"}
        resp = self.requester(
            method="GET", api="userProfile/v1/users/me/profile2", params=params
        )
    ...
    def refresh_access_token(self):
        ...
        resp = self.requester(
            method="POST", api="oauth/token", data=data, headers=headers
        )
        if resp.status_code != 200:
            return None, None
        record = resp.json()
    ...
    def get_access_token_from_npsso(self):
        if not self.npsso:
            raise Exception(
                "No npsso code, you should get a new one from "
        ...
        if resp.status_code != 302:
            raise Exception(
                "Invalid npsso code, you should get a new one from "
                "https://ca.account.sony.com/api/v1/ssocookie"
            )
    ...
    def game_list(self, offset=0, limit=200):
        resp = self.requester(
            method="GET",
            api=f"gamelist/v2/users/me/titles?limit={limit}&offset={offset}",
        )
        if resp.status_code != 200:
            return None
    ...
    def trophy_list(self, offset=0, limit=500):
        params = {"limit": limit, "offset": offset, "accountId": "me"}
        resp = self.requester(
            method="GET", api="trophy/v1/users/me/trophyTitles", params=params
        )
        if resp.status_code != 200:
            return None
        resp = resp.json()
    ...
    def game_trophy_progress(self, game_id):
        params = {"accountId": "me", "npTitleIds": game_id}
        resp = self.requester(
            method="GET", api="trophy/v1/users/me/titles/trophyTitles", params=params
        )
        resp = resp.json()
