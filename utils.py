...
def build_duration(duration_str: str) -> models.Duration:
    hrs = 0
    mins = 0
    if "hrs" in duration_str:
        hrs = int(duration_str.split("hrs")[0].strip())

    if "mins" in duration_str:
        mins = int(duration_str.split("mins")[0].strip().split(" ")[-1])

    return models.Duration(hours=hrs, minutes=mins)
